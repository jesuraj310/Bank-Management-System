...
class UserData{
	...
	public int returnIntTime(String Time){
		String[] rem_time = Time.split(":");
		int HOUR,MIN,SEC,hour,min,sec;
		SEC = Integer.parseInt(rem_time[2]);MIN = Integer.parseInt(rem_time[1]);HOUR = Integer.parseInt(rem_time[0]);
		String[] cur_time = returnTime().split(":");
		sec = Integer.parseInt(cur_time[2]);min = Integer.parseInt(cur_time[1]);hour = Integer.parseInt(cur_time[0]);
		if(HOUR == 23 && MIN ==59){
			if(hour == 0 && min ==0){
...
class thread_bms extends Thread{
	Thread t;
	public void run(){
		UserData jaddu = new UserData();
		...
		while(true){
			...
			try{
				...
				while(result.next()){
					...
					int rem_min = result.getInt("remaining_min");
					long Trans_Amount = result.getLong("trans_amount");
					if(rem_min==0){
						String Str = "DELETE FROM remainder WHERE remaining_min = 0";
						Connect.st.execute(Str);
						result = Connect.st.executeQuery(" SELECT * FROM remainder ");
					}
					long Amount_Total = jaddu.returnAmount(From);
					result = Connect.st.executeQuery(" SELECT * FROM remainder ");
					if((Amount_Total-Trans_Amount) < 0){
						//System.out.print("#");
						String Str = "DELETE FROM remainder WHERE from_ = '"+From+"' and to_ = '"+To+"' ";
						Connect.st.execute(Str);
						result = Connect.st.executeQuery(" SELECT * FROM remainder ");
						String email = jaddu.returnEmail(From);
						gmail.sendMail("hackerboytom@gmail.com",email,"Remainder not send ","Your Account Balance("+Long.toString(Amount_Total)+") Less Then Your Transaction Amount("+Long.toString(Trans_Amount)+")...\n So You Add Money to Your Account and Reset Your remainder ...");
						result = Connect.st.executeQuery(" SELECT * FROM remainder ");
					}
					else{
						if(jaddu.returnIntTime(T)==1){
							//System.out.print("*");
							Amount_Total = jaddu.returnAmount(From);
							result = Connect.st.executeQuery(" SELECT * FROM remainder ");
							if((Amount_Total-Trans_Amount) <1000){
								String email = jaddu.returnEmail(From);
								gmail.sendMail("hackerboytom@gmail.com",email,"Account Balance is Low","Your Account Balance is Lower Than MInimum Balance(1000)");
								result = Connect.st.executeQuery(" SELECT * FROM remainder ");
							}
							//System.out.print("-");
							result = Connect.st.executeQuery(" SELECT * FROM remainder ");
							String Str = "UPDATE remainder SET remaining_min = "+(rem_min-1)+",time = '"+jaddu.returnTime()+"' WHERE from_ = '"+From+"' and to_ = '"+To+"'";
							Connect.st.execute(Str);
							//System.out.print("@");
							result = Connect.st.executeQuery(" SELECT * FROM remainder ");
							jaddu.AddAmount(To,Trans_Amount);
							result = Connect.st.executeQuery(" SELECT * FROM remainder ");
							jaddu.WithdrawAmount(From,Trans_Amount);
							result = Connect.st.executeQuery(" SELECT * FROM remainder ");
							Str = "INSERT INTO history VALUES ('"+From+"','"+To+"','"+jaddu.returnDate()+"','"+jaddu.returnTime()+"','Send Amount is','"+Trans_Amount+"')";
							Connect.st.executeUpdate(Str);
							result = Connect.st.executeQuery(" SELECT * FROM remainder ");
							String str = "INSERT INTO history VALUES ('"+To+"','"+From+"','"+jaddu.returnDate()+"','"+jaddu.returnTime()+"','Receiveing Amount is','"+Trans_Amount+"')";
							Connect.st.executeUpdate(str);
							result = Connect.st.executeQuery(" SELECT * FROM remainder ");
						}
					}
	...
	public static void main(String[] args){
		Thread t = new Thread(new thread_bms());
		t.start();
								...
								do{
									...
									switch(LogIn_Choice){
										...
										case 7:
											...
											if(Acc_no==0){
												System.out.print("\n\t\t\tSuccessfully Exit From Remainder Set\n");
												break;
											}
											Reciver_User_Name = newAccount.returnUserName(Acc_no);
											if(Reciver_User_Name.equals(User_name)){
											...
											System.out.print("\n\t\t\tHow Many Times Extend the Transaction : ");
											int trans_mon = sc.nextInt();
											while(trans_mon<0){
												System.out.print("\n\t\t\t!!! Transaction Time is Lower Than Zero !!!");
												System.out.print("\n\t\t\tHow Many Times Extend the Transaction : ");
											...
											if((Amount-(Transaction_Amount*trans_mon))<0){
												System.out.print("\n\t\t\t!!!Your Account Balance was Not fullfill the Full Transaction Time So Be Alert!!!");
											}
											try{
